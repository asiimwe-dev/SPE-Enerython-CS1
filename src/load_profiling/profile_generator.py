...
def generate_load_profiles(
    ...
    # Determine IT MW and PUE from phase or explicit values
    ...
    for key, scenario in scenarios.items():
        ...
        utilization = (
            scenario.offpeak_utilization + util_range * diurnal * business_boost
        )
...
def generate_all_phase_profiles(
    config_path: Path = CONFIG_PATH,
) -> dict[str, pd.DataFrame]:
    """Generate load profiles for all 3 phases x 3 scenarios = 9 DataFrames."""
    all_profiles = {}
    for phase_key in ["phase_1", "phase_2", "phase_3"]:
        profiles = generate_load_profiles(phase_key=phase_key, config_path=config_path)
        for scenario_key, df in profiles.items():
            all_profiles[f"{phase_key}_{scenario_key}"] = df
    return all_profiles
